...
func checkWeatherCode(wea []weather.Weather) []weather.Weather {
	for i, w := range wea {
		if weather.CodeMatching(w.CondiCode, []string{"1063", "1180", "1183", "1186", "1189", "1192", "1195", "1246"}) {
			wea[i].CondiCode = "1015"
			continue
		} else if weather.CodeMatching(w.CondiCode, []string{"1072", "1150", "1153", "1168", "1171"}) {
			wea[i].CondiCode = "1018"
			continue
		} else if weather.CodeMatching(w.CondiCode, []string{"1135", "1147"}) {
			wea[i].CondiCode = "1030"
			continue
		} else if weather.CodeMatching(w.CondiCode, []string{"1240", "1243"}) {
			wea[i].CondiCode = "1027"
			continue
		} else if weather.CodeMatching(w.CondiCode, []string{"1273", "1276"}) {
			wea[i].CondiCode = "1036"
			continue
		} else if weather.CodeMatching(w.CondiCode, []string{"1087"}) {
			wea[i].CondiCode = "1021"
			continue
		} else if weather.CodeMatching(w.CondiCode, []string{"1066", "1069", "1114", "1117", "1204", "1207", "1210", "1213", "1216", "1219", "1222", "1225", "1249", "1252", "1255", "1258"}) {
			wea[i].CondiCode = "1012"
			continue
		} else if weather.CodeMatching(w.CondiCode, []string{"1198", "1201", "1237", "1261", "1264"}) {
			wea[i].CondiCode = "1024"
			continue
		} else if weather.CodeMatching(w.CondiCode, []string{"1279", "1282"}) {
